    ...
    try:#Exception handling 
        ...
        if req.status_code == 200: #Status code 200 indicates a successful request
            ...
            for n,i in enumerate(search_result): #iterating through the search results
                ...
                #Appending the result data frame after processing each individual search resul# result_df = result_df.append(pd.DataFrame({"Title": url_txt, "URL": href, "Description": description}, index=[n]))

                # result_df = pd.DataFrame({"Title": url_txt, "URL": href, "Description": description})
                # result_df.concat([result_df])
                
                # result_df =pd.concat([result_df, pd.DataFrame({"Title": url_txt, "URL": href, "Description": description})], join="inner")
                # result_df = result_df.concat([pd.DataFrame({"Title": url_txt, "URL": href, "Description": description}, index=[n])])
